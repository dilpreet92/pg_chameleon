...
class global_config(object):
    ...
    def __init__(self, config_name="default", debug_mode=False):
        ...
        try:
            ...
            self.airbrakeNotifier = Airbrake(api_key=self.airbrake_api_key, project_id=self.airbrake_project_id,
                                             environment=self.environment)
            self.airbrakeLogger = airbrake.getLogger(api_key=self.airbrake_api_key, project_id=self.airbrake_project_id,
                                                     environment=self.environment)
            self.redisHost = confdic["redis_host"]
            self.redisPort = confdic["redis_port"]
            self.redisDbIndex = confdic["redis_db_index"]
            self.redisClient = redis.StrictRedis(host=self.redisHost, port=self.redisPort, db=self.redisDbIndex)
            ...
            print('Missing key %s in configuration file. check %s/config-example.yaml for reference' % (
            key_missing, self.config_dir))
            sys.exit(4)
...
class replica_engine(object):
    ...
    def __init__(self, config, debug_mode=False, nolock=False):
        ...
        if self.global_config.log_dest == 'stdout' or self.debug_mode:
            ...
            fh = TimedRotatingFileHandler(self.global_config.log_file, when="d", interval=1,
                                          backupCount=self.global_config.log_days_keep)
    ...
    def show_status(self):
        ...
        tab_headers = ['Config file', 'Destination schema', 'Status', 'Read lag', 'Last read', 'Replay lag',
                       'Last replay']
        tab_body = []

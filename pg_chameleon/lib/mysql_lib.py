...
class mysql_engine(object):
        ...
        for binlogevent in my_stream:
            if isinstance(binlogevent, RotateEvent):
                ...
                if binlogevent.query.strip().upper() not in self.stat_skip and query_schema == self.my_schema:
                    ...
                    for token in self.sql_token.tokenised:
                        ...
                        if p_log_dict:
                            ...
                            if (binlog_file_position < log_file_name) or (
                                    binlog_file_position == binlogfile and binlogevent.packet.log_pos <= last_binlog_position):
                                continue
                            else:
                                log_dict = {'t_binlog_name': binlogfile, 'i_binlog_position': binlogevent.packet.log_pos}
                                self.redisClient.set(table_name, pickle.dumps(log_dict))
                        else:
                            log_dic = {'t_binlog_name': binlogfile, 'i_binlog_position': binlogevent.packet.log_pos}
                        ...
                        if table_name in inc_tables:
                            ...
                            if write_ddl:
                                self.logger.info(
                                    ...
                                    table_name, binlogfile, log_position))
                ...
                for row in binlogevent.rows:
                    ...
                    last_binlog_position = log_dict['i_binlog_position']
                    if (binlog_file_position < log_file_name) or (
                            binlog_file_position == log_file_name and binlogevent.packet.log_pos <= last_binlog_position):
                        continue
                    else:
                    ...
                    if table_name in inc_tables:
                        ...
                        if table_consistent:
                            add_row = True
                            self.logger.debug("CONSISTENT POINT FOR TABLE %s REACHED  - binlogfile %s, position %s" % (
                            table_name, binlogfile, log_position))
                            pg_engine.set_consistent_table(table_name)
                            inc_tables = pg_engine.get_inconsistent_tables()
                        else:
                    ...
                    if len(group_insert) >= self.replica_batch_size:
                        self.logger.debug("Max rows per batch reached. Writing %s. rows. Size in bytes: %s " % (
    ...
    """
        pg_engine.pg_conn.pgsql_cur.execute(
            redshift_copy % (
            pg_engine.dest_schema + '.' + ins_arg[1] + '_new', self.aws_bucket, ins_arg[1], pg_engine.aws_key,
            pg_engine.aws_secret))
    ...
    """
      ...
      COPY %s from 's3://%s/%s' credentials 'aws_access_key_id=%s;aws_secret_access_key=%s' csv TRUNCATECOLUMNS;
    """
        pg_engine.pg_conn.pgsql_cur.execute(redshift_copy % (
        pg_engine.dest_schema + '.' + ins_arg[1], self.aws_bucket, ins_arg[1], pg_engine.aws_key, pg_engine.aws_secret))

    def read_replica_for_table(self, init_table_name, table_binlog_position, last_binlog_position, t_binlog_name,
                               pg_engine, batch_data):
        table_type_map = self.get_table_type_map()
        schema_name = pg_engine.dest_schema
        inc_tables = pg_engine.get_inconsistent_tables()
        close_batch = False
    ...
    """
        ...
        for i in init_results:
            ...
            if (last_binlog_file_name > table_binlog_file_name) or (
                    last_binlog_file_name == table_binlog_file_name and last_binlog_position > i[3]):
    ...
    def copy_table_data_table(self, pg_engine, copy_max_memory, table_name, lock_tables=True):
      ...
      """
            ...
            while True:
                self.logger.debug("%s will be copied from primary key starting from %s slices of %s rows" % (
                table_name, primary_key_count, total_rows))
                csv_data = ""
                sql_out = "SELECT " + columns_csv + " as data FROM " + table_name + " WHERE id >= " + str(
                    primary_key_count) + " ORDER BY id LIMIT " + str(copy_limit) + ";"
                try:
                ...
                try:
                    ...
                    self.logger.info(
                        "table %s error in PostgreSQL copy, saving slice number for the fallback to insert statements " % (
                        table_name,))
